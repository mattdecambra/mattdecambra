...
dams = read_excel("/Users/mikejohnson/github/dams/data/NID2019.xlsx") %>% 
...
fire = damns %>% 
  filter(grepl("P",damns$PURPOSES)) %>% 
  polycounts(veroni)
hydro = damns %>% 
  filter(grepl("H",damns$PURPOSES)) %>% 
  polycounts(veroni)
plot_pip(hydro)
 
polycounts = function(polygons,points){
  counter = st_intersects(polygons, points)
}

hydro_piip = polycounts(sq_grid, Rec) 
plot_pip(hydro_piip)
